...
#no rules found for the West

MidwestRules <- subset(RulesName, items %in% "us_region=Midwest")

Recurring <- subset(RulesName, items %in% "donor_type=recurring")
#no rules found for recurring

#plot top 20 rules
plot(RulesName[1:20], method="graph", control=list(), measure = "count", shading = "confidence")
plot(MidwestRules[1:20], method="graph", control=list(), measure = "count", shading = "confidence")
plot(SouthRules[1:20], method="graph", control=list(), measure = "count", shading = "confidence")
